...
#[allow(dead_code)]
pub fn dimension_prefix_sum_violence(
    matrix: &Vec<Vec<i32>>,
    left_top_point: (i32, i32),
    right_bottom_point: (i32, i32),
) -> i32 {
    let mut sum = 0;
...
#[allow(dead_code)]
pub fn dimensiom_prefix_sum_optmize(
    matrix: &Vec<Vec<i32>>,
    left_top_point: (i32, i32),
    right_bottom_point: (i32, i32),
) -> i32 {
    ...
    for i in 1..width {
        for j in 1..length {
            record_matrix[i][j] =
                record_matrix[i - 1][j] + record_matrix[i][j - 1] - record_matrix[i - 1][j - 1];
        }
    }
